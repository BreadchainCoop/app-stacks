...
import { useCollectiveFund, CollectiveFund } from "@/hooks/use-collective-fund";
...
export default function CollectiveInvite() {
  const searchParams = useSearchParams();
  const parsedId = BigInt(searchParams.get("circleId") || "0");

  const fundResult = useCollectiveFund(parsedId);
  ...
  return (
    <div className="*:mb-6 last:mb-0 page-layout py-6 w-full max-w-142 mx-auto card-shadow-bg">
      <div className="flex flex-col text-center items-center justify-center gap-3">
        ...
        <Body className="">
          Accept this invite to join this collective fund.
        </Body>
      </div>

      {fundResult.data ? (
        <>
          <CollectiveInviteDetails fund={fundResult.data} />
...
function CollectiveInviteDetails({ fund }: { fund: CollectiveFund }) {
  const searchParams = useSearchParams();
  const fundId = searchParams.get("circleId") as string;
  const members = searchParams.get("members");

  // The fund name is stored on-chain (white-label), so prefer it over the
  // invite-URL param.
  const fundName = fund.name || searchParams.get("name") || "-";

  return (
    <div className="border-t border-blue-0 pt-6">
      <Body className="text-center mb-6">
        You have been invited to join the &quot;{fundName}&quot; collective
        fund.
      </Body>
        ...
        >
          ...
          <AccordionContent>
            <div className="">
              ...
              <RowDetail
                label="Pool balance"
                body={`${formatBalance(+formatEther(fund.poolBalance), 2)} BREAD`}
              />
              <RowDetail
                label="Voting period"
                body={formatSecondsHuman(Number(fund.votingPeriod))}
              />
              <RowDetail
                label="Approval threshold"
                body={`${formatBps(fund.approvalThresholdBps)}% of members`}
              />
            </div>
          </AccordionContent>
        </AccordionItem>
...
function CollectiveAcceptInvite() {
    ...
    try {
      ...
      try {
        const res = await fetch("/api/stacks/invite", {
          ...
          body: JSON.stringify({
            circleId: stackMetadataId("collective", fundId),
            nonce,
        ...
        if (!res.ok) {
          console.error(
            "collective-invite: /api/stacks/invite PATCH failed",
            res.status,
        ...
        console.error(
          "collective-invite: /api/stacks/invite PATCH error",
          patchErr
        );
      }

      router.push(stackTypeDetailPath("collective", fundId));
    } catch (error) {
      alert(
        parseContractError(
          error,
          { ...COLLECTIVE_FUND_ERRORS, ...COLLECTIVE_JOIN_ERRORS },
