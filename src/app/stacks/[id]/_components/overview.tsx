	...
	return (
		<section className="bg-paper-0 p-5 mb-4 *:mb-4 md:mb-0 md:order-1 md:flex-2 md:max-w-159.75">
			<header className="flex items-center justify-between flex-wrap gap-2">
				<Heading3 className="text-2xl shrink-0">
					Deposit overview
				</Heading3>
				<Body bold className="text-xs shrink-0">
					<span className="font-normal">Stack status: </span>
					<span>
						{roundsCompleted} out of {circle.totalRounds} rounds
						complete
					</span>
				</Body>
			</header>
			<div className="flex flex-col gap-4 border-b border-paper-2 pb-4 md:flex-row md:justify-between">
				<Body className="flex flex-col justify-start md:items-start">
					...
					<span>
						<span className="font-bold">{membersDeposited}</span>{" "}
						out of <span className="font-bold">{totalMembers}</span>{" "}
						members
					</span>
				</Body>
					...
					<span
						className={cn(
							...
							[
								...
								"claimable",
								"deposit-completed",
							].includes(formattedCircleStatus.status) &&
								"text-system-green",
						)}
					>
			...
			<DaysLeft
				...
				effectiveCircleStartTime={
					circle.circleInfo.effectiveCircleStartTime
				}
				  ...
				  circle.isMember ? (
					<>
						{connectedUser.user.status === "CONNECTED" ? (
							...
							>
								Stack Failed - Retire group & Claim your
								deposits
							</LiftedButton>
						) : (
							<LoginButton
