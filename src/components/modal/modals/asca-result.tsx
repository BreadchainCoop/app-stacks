...
function buildInviteUrl(
  ...
  url.searchParams.set("type", "asca");
  url.searchParams.set("contract", ASCA_CONTRACT_ADDRESS);
...
export const AscaSuccessResultModal = ({
  modalState,
}: {
  modalState: AscaCreationSuccessModalState;
}) => {
  ...
  const generateInvites = async () => {
    ...
    try {
      ...
      for (let i = 0; i < invitePayloads.length; i++) {
        ...
        const typedData = {
          domain: {
            ...
            verifyingContract: ASCA_CONTRACT_ADDRESS,
          },
          types: {
            Invite: [
      ...
      fetch("/api/stacks/metadata", {
        ...
        body: JSON.stringify({
          id: stackMetadataId("asca", modalState.fund.id),
  ...
  return (
    <ModalContainer className="max-w-142!">
      <div className="flex flex-col gap-3 items-center justify-center">
        ...
        <Body className="text-surface-ink">Savings & credit fund created!</Body>
      </div>
          ...
          <AccordionItem value="detail">
            ...
            <AccordionContent>
              <div>
                ...
                <RowDetail
                  label="Borrow limit"
                  body={`${formatBps(modalState.fund.borrowLimitBps)}% of savings`}
                />
                <RowDetail
                  label="Interest rate"
                  body={`${formatBps(modalState.fund.interestRateBps)}% per period`}
                />
                <RowDetail
                  label="Period length"
                  body={formatSecondsHuman(
                    Number(modalState.fund.periodLength)
                  )}
                />
                <RowDetail
                  label="Loan due after"
                  body={`${modalState.fund.repaymentPeriods} ${
                    modalState.fund.repaymentPeriods === 1
                      ? "period"
                      : "periods"
                  }`}
                />
      ...
      <LocalButton
        as={Link}
        href={stackTypeDetailPath("asca", modalState.fund.id)}
        className="w-full"
...
export const AscaFailedResultModal = ({
  modalState,
}: {
  modalState: AscaCreationFailedModalState;
}) => {
  return (
