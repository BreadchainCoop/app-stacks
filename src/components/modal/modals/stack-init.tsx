...
import { StackInitModalState } from "../context";
...
const StackInitModal = ({
	modalState,
}: {
	modalState: StackInitModalState;
}) => {
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			setCurrentStep((prev) => {
				...
				// We finalized the last check list when creating stack is successful
				if (
					prev === steps.length - 1 &&
					modalState.status !== "successful"
				) {
					return prev;
				}
		...
		<ModalContainer>
			...
			<div className="text-center">
				<Heading3 className="text-2xl mb-3">
					Creating your Stacks group
				</Heading3>
				<Body className="text-surface-grey-2 mb-6">
					Please wait while set up your Stacks group..
				</Body>
