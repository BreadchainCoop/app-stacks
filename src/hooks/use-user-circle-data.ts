...
export function useUserCircleData(circleId: bigint | undefined) {
  ...
  const { data: circleData, isLoading, error, refetch } = useReadContract({
    address: SAVING_CIRCLES_VIEWER_CONTRACT_ADDRESS,
    abi: savingCirclesViewerAbi,
    functionName: 'getUserCircleData',
    args: address && circleId !== undefined ? [address, circleId] : undefined,
    query: {
      enabled: isConnected && address !== undefined && circleId !== undefined,
    },
  });
